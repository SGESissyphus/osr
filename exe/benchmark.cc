...
std::vector<location> parse_locations_from_file(const fs::path file_name) {
  ...
  for (const auto& query : json_data.as_object().at("queries").as_array()) {
    // Parse start point
    location start_loc;
    start_loc.pos_.lat_ = query.as_object().at("start_point").as_object().at("lat").as_double();
    start_loc.pos_.lng_ = query.as_object().at("start_point").as_object().at("lng").as_double();
    locations.push_back(start_loc);

    // Parse end point
    location end_loc;
    end_loc.pos_.lat_ = query.as_object().at("end_point").as_object().at("lat").as_double();
    end_loc.pos_.lng_ = query.as_object().at("end_point").as_object().at("lng").as_double();
