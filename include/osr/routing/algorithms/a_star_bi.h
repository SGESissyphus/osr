    ...
    while (!minHeap1_.empty() && !minHeap2_.empty()) {
      ...
      if (curr1 != std::nullopt) {
        if (!expanded_.contains(curr1.value())) {
          std::cout << "curr1 adds in expand: " << static_cast<std::uint32_t>(curr1.value().n_) << std::endl;
          expanded_.emplace(curr1.value());
        } else {
          meet_point = curr1.value();
          std::cout << "breaking because curr1 is already expanded:" << static_cast<std::uint32_t>(meet_point.n_) << std::endl;
          return;
        }
      }
      if (curr2 != std::nullopt) {
        if (!expanded_.contains(curr2.value())) {
          std::cout << "curr2 adds in expand: " << static_cast<std::uint32_t>(curr2.value().n_) << std::endl;
          expanded_.emplace(curr2.value());
        } else {
          meet_point = curr2.value();
          std::cout << "breaking because curr2 is already expanded:" << static_cast<std::uint32_t>(meet_point.n_) << std::endl;
          return;
        }
      }
    }
