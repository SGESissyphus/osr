        ...
        if (nc->valid() && nc->cost_ < max) {
          Profile::resolve_start_node(
              *w.r_, end.way_, nc->node_, to.lvl_, opposite(dir),
              [&](auto const node) { a.add_start({node, nc->cost_}, false);
              });
        }
      }
    }
...
std::optional<path> route_a_star_bi(ways const& w,
  ...
  switch (profile) {
    case search_profile::kFoot:
      return route(w, l, get_a_star_bi<foot<false, elevator_tracking>>(), from, to,
                   max, dir, max_match_distance, blocked);
    case search_profile::kWheelchair:
      return route(w, l, get_a_star_bi<foot<true, elevator_tracking>>(), from, to,
                   max, dir, max_match_distance, blocked);
    case search_profile::kBike:
    ...
    case search_profile::kCar:
      return route(w, l, get_a_star_bi<car>(), from, to, max, dir,
                   max_match_distance, blocked);
    case search_profile::kCarParking:
      return route(w, l, get_a_star_bi<car_parking<false>>(), from, to, max, dir,
                   max_match_distance, blocked);
    case search_profile::kCarParkingWheelchair:
